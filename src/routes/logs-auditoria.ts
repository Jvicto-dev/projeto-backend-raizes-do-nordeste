...
export async function logsAuditoriaRoutes(app: FastifyInstance) {
  app.get(
    ...
    async (request, reply) => {
      ...
      let base = db('logs_auditoria as la')
        .leftJoin('usuarios as u', 'la.usuario_id', 'u.id')
        .modify((qb) => {
          if (usuario_id) {
            void qb.where('la.usuario_id', usuario_id)
          }
          if (acao) {
            void qb.where('la.acao', acao)
          }
          if (desde) {
            void qb.where('la.timestamp', '>=', new Date(desde))
          }
          if (ate) {
            void qb.where('la.timestamp', '<=', new Date(ate))
          }
        })
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      return reply.status(200).send(serializeRow(row as Record<string, unknown>))
    }
  )
}
