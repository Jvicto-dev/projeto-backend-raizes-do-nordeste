...
function serializeCampanha(row: Record<string, unknown>) {
  const vd = row.valido_de instanceof Date ? row.valido_de.toISOString() : String(row.valido_de)
  const va = row.valido_ate instanceof Date ? row.valido_ate.toISOString() : String(row.valido_ate)
  return {
    id: row.id,
    nome: row.nome,
...
export async function campanhasRoutes(app: FastifyInstance) {
  app.get(
      ...
      schema: {
        ...
        querystring: {
          ...
          properties: {
            ...
            ativas: { type: 'boolean', description: 'Se true, apenas campanhas ativas no periodo atual' },
    ...
    async (request, reply) => {
      ...
      if (q.ativas === true) {
        const now = db.fn.now()
        countQuery = countQuery.where({ ativa: true }).where('valido_de', '<=', now).where('valido_ate', '>=', now)
        listQuery = listQuery.where({ ativa: true }).where('valido_de', '<=', now).where('valido_ate', '>=', now)
      }
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      return reply.status(200).send(serializeCampanha(row as Record<string, unknown>))
    }
  )
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      if (pb.data.unidade_id) {
        const u = await db('unidades').select('id').where({ id: pb.data.unidade_id }).first()
        if (!u) {
          return reply.status(404).send({
            error: 'NAO_ENCONTRADO',
      ...
      return reply.status(201).send(serializeCampanha(row as Record<string, unknown>))
    }
  )
  ...
  app.put(
    ...
    async (request, reply) => {
      ...
      const vd =
        pb.data.valido_de !== undefined ? new Date(pb.data.valido_de) : new Date(String(existing.valido_de))
      const va =
        pb.data.valido_ate !== undefined ? new Date(pb.data.valido_ate) : new Date(String(existing.valido_ate))
      if (vd >= va) {
        return reply.status(400).send({
          error: 'DADOS_INVALIDOS',
          message: 'valido_de deve ser anterior a valido_ate.'
      ...
      if (pb.data.percentual_desconto !== undefined) patch.percentual_desconto = pb.data.percentual_desconto
      ...
      if (pb.data.unidade_id !== undefined) patch.unidade_id = pb.data.unidade_id
