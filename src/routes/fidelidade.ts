...
function isAdminOrGerente(request: { user?: unknown }): boolean {
...
export async function fidelidadeRoutes(app: FastifyInstance) {
  app.get(
    ...
    async (request, reply) => {
      ...
      if (typeof q.consentimento_explicitado === 'boolean') {
        countQuery = countQuery.where({ consentimento_explicitado: q.consentimento_explicitado })
        listQuery = listQuery.where({ consentimento_explicitado: q.consentimento_explicitado })
      }
      ...
      return reply.status(200).send({
        data: rows.map((r) => serializeFidelidade(r as Record<string, unknown>)),
        page,
        limit,
        total
      })
    ...
    '/fidelidade/:id',
    {
      ...
      schema: {
        ...
        response: {
          ...
          401: { description: 'Token invalido/ausente', ...errorResponseSchema },
          403: { description: 'Sem permissao', ...errorResponseSchema },
          404: { description: 'Registro nao encontrado', ...errorResponseSchema }
        }
      }
    },
    async (request, reply) => {
      ...
      if (!isAdminOrGerente(request) && String(row.cliente_id) !== sub) {
        return reply.status(403).send(forbiddenError())
      }

      return reply.status(200).send(serializeFidelidade(row as Record<string, unknown>))
    }
  )
    ...
    '/fidelidade',
    {
      ...
      schema: {
        ...
        response: {
          ...
          401: { description: 'Token invalido/ausente', ...errorResponseSchema },
          403: { description: 'Sem permissao', ...errorResponseSchema },
          404: { description: 'Cliente nao encontrado', ...errorResponseSchema },
          409: { description: 'Cliente ja cadastrado na fidelidade', ...errorResponseSchema }
        }
      }
    },
    async (request, reply) => {
      ...
      const cliente = await db('usuarios').select('id').where({ id: pb.data.cliente_id }).first()
      if (!cliente) {
        return reply.status(404).send({ error: 'NAO_ENCONTRADO', message: 'Cliente nao encontrado.' })
      }

      const existing = await db('fidelidade').where({ cliente_id: pb.data.cliente_id }).first()
      if (existing) {
      ...
      const id = randomUUID()
      const consent = pb.data.consentimento_explicitado ?? false
      const dataConsent = consent ? (pb.data.data_consentimento ?? new Date().toISOString()) : null

      await db('fidelidade').insert({
        id,
        cliente_id: pb.data.cliente_id,
        saldo_pontos: pb.data.saldo_pontos ?? 0,
    ...
    async (request, reply) => {
      ...
      const bodySchema = z
        .object({
          ...
          consentimento_explicitado: z.boolean().optional(),
          data_consentimento: z.union([z.string().datetime(), z.null()]).optional()
        })
        .refine(
          (d) =>
      ...
      const row = await db('fidelidade').where({ id: pp.data.id }).first()
      if (!row) {
        return reply.status(404).send({ error: 'NAO_ENCONTRADO', message: 'Fidelidade nao encontrada.' })
      }
      ...
      if (pb.data.saldo_pontos !== undefined) {
        saldoFinal = pb.data.saldo_pontos
      }
      if (pb.data.ajuste_pontos_delta !== undefined) {
        saldoFinal += pb.data.ajuste_pontos_delta
      }
      if (saldoFinal < 0) {
        return reply.status(409).send({
          error: 'CONFLITO',
      ...
      if (pb.data.consentimento_explicitado !== undefined) {
        patch.consentimento_explicitado = pb.data.consentimento_explicitado
        if (pb.data.consentimento_explicitado === false) {
          patch.data_consentimento = null
        } else if (pb.data.data_consentimento === undefined && !row.data_consentimento) {
          patch.data_consentimento = new Date().toISOString()
        }
      }
      if (pb.data.data_consentimento !== undefined) {
        patch.data_consentimento = pb.data.data_consentimento
      }
  ...
  app.delete(
    ...
    async (request, reply) => {
      ...
      const deleted = await db('fidelidade').where({ id: pp.data.id }).del()
      if (deleted === 0) {
        return reply.status(404).send({ error: 'NAO_ENCONTRADO', message: 'Fidelidade nao encontrada.' })
      }
